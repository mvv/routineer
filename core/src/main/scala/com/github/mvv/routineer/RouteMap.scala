...
trait RouteMap[K, Req, +Resp] {
  ...
  def ++[Resp1 >: Resp](map: RouteMap[K, Req, Resp1]): RouteMap[K, Req, Resp1] = {
  ...
  override def toString: String =
    routesByKey.foldLeft("(route-map") {
      case (str, (key, rs)) =>
        str + "\n  (:" + key + "\n" +
          "     (" + rs + ")"
    } + ")"
...
object RouteMap {
  final private class MapBacked[K, Req, +Resp](protected val routesByKey: Map[K, Routes[Req, Resp]])
...
trait DeclRouteMap[K, Req, Resp] extends RouteMap[K, Req, Resp] {
  ...
  final protected def routesByKey: Map[K, Routes[Req, Resp]] = routesVar

  final protected def routeDecl[Elems <: PathSpec.Elems](key: K, spec: PathSpec[Elems]) =
    new SimpleRouteDecl(this, key, spec)
}

object DeclRouteMap {
  private def addRoute[K, Req, Resp](map: DeclRouteMap[K, Req, Resp], key: K, route: Route[Req, Resp]): Unit =
  ...
  final protected class SimpleRouteDecl[K, Req, Resp, Elems <: PathSpec.Elems] private[DeclRouteMap] (
      map: DeclRouteMap[K, Req, Resp],
      key: K,
      spec: PathSpec[Elems]
  ) {
    def apply[R](body: Elems#Prepend[Req]#Func[R])(implicit hasNext: Elems#HasNext =:= PathSpec.TTrue,
    ...
    def onlyIf(cond: Elems#Prepend[Req]#Func[Boolean]): CondRouteDecl[K, Req, Resp, Elems] =
      new CondRouteDecl(map, key, spec, cond)
    def guard[G](guard: Elems#Prepend[Req]#Func[Option[G]]): GuardedRouteDecl[K, Req, Resp, Elems, G] =
      new GuardedRouteDecl(map, key, spec, guard)
  }
  final protected class CondRouteDecl[K, Req, Resp, Elems <: PathSpec.Elems] private[DeclRouteMap] (
      map: DeclRouteMap[K, Req, Resp],
      key: K,
      spec: PathSpec[Elems],
  ...
  final protected class GuardedRouteDecl[K, Req, Resp, Elems <: PathSpec.Elems, G] private[DeclRouteMap] (
      map: DeclRouteMap[K, Req, Resp],
      key: K,
      spec: PathSpec[Elems],
      guard: Elems#Prepend[Req]#Func[Option[G]]
  ) {
    def apply[R](body: Elems#Prepend[Req]#Append[G]#Func[R])(implicit hasNext: Elems#Prepend[Req]#HasNext =:=
                                                               PathSpec.TTrue,
                                                             ops: PathSpec.ElemsOps[Elems],
                                                             prepOps: PathSpec.ElemsOps[Elems#Prepend[Req]],
