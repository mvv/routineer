...
case class RouteOvershadowedException[Req, Resp](route: Route[Req, Resp], by: Route[Req, Resp]) extends Exception {
...
sealed trait Route[Req, +Resp] extends Routes.NonEmpty[Req, Resp] {
  ...
  protected[routineer] def apply(evaluator: Routes.PatternEvaluator, path: Routes.Path, args: Seq[Any]) =
...
)(implicit hasNext: Elems#HasNext =:= PathSpec.TTrue, ops: PathSpec.ElemsOps[Elems#Prepend[Req]])
...
final case class GuardedRoute[Req, +Resp, Elems <: PathSpec.Elems, G](
  ...
  prepApOps: PathSpec.ElemsOps[Elems#Prepend[Req]#Append[G]])
    extends Route[Req, Resp] {
  type RouteElems = Elems
  def isGuarded = true
  def checkGuard(args: RouteArgs) =
...
object Routes {
  private def seqToTuple(seq: Seq[Any]): Any = seq match {
    case Seq()                               => ()
    case Seq(_1)                             => _1
    case Seq(_1, _2)                         => (_2, _1)
    case Seq(_1, _2, _3)                     => (_3, _2, _1)
    case Seq(_1, _2, _3, _4)                 => (_4, _3, _2, _1)
    case Seq(_1, _2, _3, _4, _5)             => (_5, _4, _3, _2, _1)
    case Seq(_1, _2, _3, _4, _5, _6)         => (_6, _5, _4, _3, _2, _1)
    case Seq(_1, _2, _3, _4, _5, _6, _7)     => (_7, _6, _5, _4, _3, _2, _1)
    case Seq(_1, _2, _3, _4, _5, _6, _7, _8) => (_8, _7, _6, _5, _4, _3, _2, _1)
  }
  ...
  object Path {
    ...
    final class Cons(val head: String, lazyTail: => Path, val string: String) extends NonEmpty {
    ...
    def parse(str: String): Path = {
      val seps = str.segmentLength(_ == '/', 0)
      if (str.length == seps)
        Empty
      else {
        ...
        if (i < 0)
          ...
          new Cons(str.substring(seps, i), parse(str.substring(i + 1)), str.substring(seps))
      }
    }
  }

  final class PatternEvaluator() {
    private var cache: Map[(String, Pattern[String, _]), Option[Any]] = Map.empty
  ...
  final class Empty[Req, +Resp]() extends Routes[Req, Resp] {
    ...
    protected[routineer] def apply(evaluator: PatternEvaluator, path: Path, args: Seq[Any]) = None
  }
  ...
  final case class Prefixed[Req, +Resp](prefix: String, routes: Routes[Req, Resp]) extends NonEmpty[Req, Resp] {
    def ++[Resp1 >: Resp](routes1: Routes[Req, Resp1]) = routes1 match {
      ...
      case routes1 @ Straight(prefix1, end1) =>
        prefix1.headOption match {
          case Some(Left(str)) =>
            if (str == prefix)
              Prefixed(prefix, routes ++ routes1.step)
            else
              Choice(stringMap = Map(prefix -> routes, str -> routes1.step))
          case Some(Right(pat)) =>
            val pair = pat -> routes1.step
            Choice(stringMap = Map(prefix -> routes), patternMap = Map(pair), patternSeq = Vector(pair))
          case None =>
            end1.spec.rest match {
              case Some(pat1) =>
                val pair1 = pat1 -> end1
                Choice(stringMap = Map(prefix -> routes), restMap = Map(pair1), restSeq = Vector(pair1))
              case None =>
                Choice(stringMap = Map(prefix -> routes), ends = Vector(end1))
            }
        }
    ...
    protected[routineer] def apply(evaluator: PatternEvaluator, path: Path, args: Seq[Any]) =
      path match {
  ...
  final case class Straight[Req, +Resp](prefix: Seq[Either[String, Pattern[String, _]]], end: Route[Req, Resp])
      extends NonEmpty[Req, Resp] {
    def ++[Resp1 >: Resp](routes1: Routes[Req, Resp1]) = routes1 match {
      case _: Empty[_, _] => this
      case route: Route[_, _] =>
        this ++ route.asInstanceOf[Route[Req, Resp1]].straight
      case Prefixed(prefix1, routes1) =>
        prefix.headOption match {
          ...
          case Some(Right(pat)) =>
            val pair = pat -> step
            Choice(stringMap = Map(prefix1 -> routes1), patternMap = Map(pair), patternSeq = Vector(pair))
          case None =>
            end.spec.rest match {
              case Some(pat) =>
                val pair = pat -> end
                Choice(stringMap = Map(prefix1 -> routes1), restMap = Map(pair), restSeq = Vector(pair))
              case None =>
                Choice(stringMap = Map(prefix1 -> routes1), ends = Vector(end))
            }
        }
      case routes1 @ Straight(prefix1, end1) =>
        (prefix.headOption, prefix1.headOption) match {
          case (None, None) =>
            (end.spec.rest, end1.spec.rest) match {
              case (None, None) =>
                if (!end.isGuarded)
              ...
              case (None, Some(pat1)) =>
                val pair1 = pat1 -> end1
                Choice(restMap = Map(pair1), restSeq = Vector(pair1), ends = Vector(end))
              case (Some(pat), None) =>
                val pair = pat -> end
                Choice(restMap = Map(pair), restSeq = Vector(pair), ends = Vector(end1))
              case (Some(pat), Some(pat1)) =>
                ...
                if (pat == pat1) {
                  ...
                  Choice(restMap = Map(pair, pair1), restSeq = Vector(pair, pair1))
            }
          case (None, Some(Left(str1))) =>
            val map1 = Map(str1 -> routes1.step)
              ...
              case Some(pat) =>
                val pair = pat -> end
                Choice(stringMap = map1, restMap = Map(pair), restSeq = Vector(pair))
              case None =>
          ...
          case (None, Some(Right(pat1))) =>
            val pair1 = pat1 -> routes1.step
              ...
              case Some(pat) =>
                val pair = pat -> end
                Choice(patternMap = Map(pair1), patternSeq = Vector(pair1), restMap = Map(pair), restSeq = Vector(pair))
              case None =>
                Choice(patternMap = Map(pair1), patternSeq = Vector(pair1), ends = Vector(end))
            }
          case (Some(Left(str)), None) =>
            ...
            end1.spec.rest match {
              case Some(pat1) =>
                val pair1 = pat1 -> end1
                Choice(stringMap = map, restMap = Map(pair1), restSeq = Vector(pair1))
              case None =>
          ...
          case (Some(Left(str)), Some(Right(pat1))) =>
            val pair1 = pat1 -> routes1.step
            Choice(stringMap = Map(str -> step), patternMap = Map(pair1), patternSeq = Vector(pair1))
          case (Some(Right(pat)), None) =>
            val pair = pat -> step
            end1.spec.rest match {
              case Some(pat1) =>
                val pair1 = pat1 -> end1
                Choice(patternMap = Map(pair), patternSeq = Vector(pair), restMap = Map(pair1), restSeq = Vector(pair1))
              case None =>
                Choice(patternMap = Map(pair), patternSeq = Vector(pair), ends = Vector(end1))
            }
          case (Some(Right(pat)), Some(Left(str1))) =>
            val pair = pat -> step
            Choice(stringMap = Map(str1 -> routes1.step), patternMap = Map(pair), patternSeq = Vector(pair))
          case (Some(Right(pat)), Some(Right(pat1))) =>
            val pair = pat -> step
            val pair1 = pat1 -> routes1.step
            if (pat == pat1)
              Choice(patternMap = Map(pat -> (step ++ routes1.step)), patternSeq = Vector(pair, pair1))
            else
              Choice(patternMap = Map(pair, pair1), patternSeq = Vector(pair, pair1))
        }
      case _choice: Choice[_, _] =>
        val choice = _choice.asInstanceOf[Choice[Req, Resp1]]
        prefix.headOption match {
          case Some(Left(str)) =>
            choice.stringMap.get(str) match {
              case Some(routes1) =>
                choice.copy(
                  stringMap = choice.stringMap +
                    (str -> (step ++ routes1))
          ...
          case Some(Right(pat)) =>
            choice.patternMap.get(pat) match {
              case Some(routes1) =>
                choice.copy(patternMap = choice.patternMap + (pat -> (step ++ routes1)),
                            patternSeq = choice.patternSeq :+ (pat -> step))
              case None =>
                val pair = pat -> step
                choice.copy(patternMap = choice.patternMap + pair, patternSeq = choice.patternSeq :+ pair)
            }
          case None =>
            end.spec.rest match {
              case Some(pat) =>
                ...
                val pair = pat -> end
                choice.copy(restMap = choice.restMap + pair, restSeq = choice.restSeq :+ pair)
              case None =>
          ...
          path match {
            ...
            case path: Path.NonEmpty =>
              ...
              prefixHead match {
                case Left(str) =>
                  if (str == pathHead)
                    (new Straight(prefix.tail, end))(evaluator, path.tail, args)
                  else
                    None
                case Right(pat) =>
                  evaluator.eval(pat, pathHead) match {
                    case Some(arg) =>
                      (new Straight(prefix.tail, end))(evaluator, path.tail, arg +: args)
                    case None =>
                      None
                  }
              }
        ...
        case None =>
            ...
            case Some(pat) =>
              path match {
                case Path.Empty => None
                case path: Path.NonEmpty =>
                  pat.matches(path.string) match {
                    case Some(arg) =>
                      end.checkGuard(seqToTuple(arg +: args).asInstanceOf[end.RouteArgs])
                    case None => None
                  }
              }
            case None =>
  ...
  final case class Choice[Req, +Resp](
      stringMap: Map[String, Routes[Req, Resp]] = Map.empty[String, Routes[Req, Resp]],
      patternMap: Map[Pattern[String, _], Routes[Req, Resp]] = Map.empty[Pattern[String, _], Routes[Req, Resp]],
      patternSeq: Seq[(Pattern[String, _], Routes[Req, Resp])] = Vector.empty[(Pattern[String, _], Routes[Req, Resp])],
      restMap: Map[Pattern[String, _], Route[Req, Resp]] = Map.empty[Pattern[String, _], Route[Req, Resp]],
      restSeq: Seq[(Pattern[String, _], Route[Req, Resp])] = Vector.empty[(Pattern[String, _], Route[Req, Resp])],
    ...
    def ++[Resp1 >: Resp](routes1: Routes[Req, Resp1]) = routes1 match {
      ...
      case routes1 @ Straight(prefix1, end1) =>
        prefix1.headOption match {
          ...
          case Some(Right(pat1)) =>
            patternMap.get(pat1) match {
              case Some(routes) =>
                val step1 = routes1.step
                copy(patternMap = patternMap + (pat1 -> (routes ++ step1)), patternSeq = patternSeq :+ (pat1 -> step1))
              case None =>
                val pair1 = pat1 -> routes1.step
                copy(patternMap = patternMap + pair1, patternSeq = patternSeq :+ pair1)
            }
          case None =>
            end1.spec.rest match {
              case Some(pat1) =>
      ...
      case _choice: Choice[_, _] =>
        val choice = _choice.asInstanceOf[Choice[Req, Resp1]]
        val newStringMap =
          choice.stringMap.foldLeft(stringMap: Map[String, Routes[Req, Resp1]]) {
            case (m, (str1, routes1)) =>
              m.get(str1) match {
        ...
        val (newPatternMap, newPatternSeq) =
          choice.patternSeq.foldLeft(
            (patternMap: Map[Pattern[String, _], Routes[Req, Resp1]],
             patternSeq: Seq[(Pattern[String, _], Routes[Req, Resp1])])
          ) {
            case ((m, s), (pat1, routes1)) =>
              m.get(pat1) match {
                case Some(routes) =>
        ...
        val (newRestMap, newRestSeq) =
          choice.restSeq.foldLeft(
            (restMap: Map[Pattern[String, _], Route[Req, Resp1]], restSeq: Seq[(Pattern[String, _], Route[Req, Resp1])])
          ) {
            case ((m, s), (pat1, end1)) =>
    ...
    protected[routineer] def apply(evaluator: PatternEvaluator, path: Path, args: Seq[Any]): Option[() => Resp] =
      path match {
        case Path.Empty =>
          ends.foreach { end =>
            val result = end.checkGuard(seqToTuple(args).asInstanceOf[end.RouteArgs])
            if (result.isDefined)
              return result
          }
          return None
